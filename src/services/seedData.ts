...
const referralTypes: ('self' | 'external' | 'internal')[] = ['self', 'external', 'internal'];

const generateRequirements = (status: 'needs-review' | 'scheduled' | 'rejected') => {
...
const generateDocuments = (isReviewed: boolean) => [
    {
        type: "MRI Report",
...
const generateNotes = (surgeryType: string, provider: string) => {
    const notes = [];
    const noteTypes: ('general' | 'surgical' | 'medical' | 'requirement')[] = 
        ['general', 'surgical', 'medical', 'requirement'];
